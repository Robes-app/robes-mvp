...
- **Three result sections form "The edit."** (in the Edit tab): Keep (owned cards — **Packed** checkbox + Swap), Worth adding (unowned cards — a dark **+ Add** button + Swap, NOT a pack checkbox), Leave behind (compact muted rows). `+ Add` → `__tvAddOwn` → the reframed own-prompt (Snap it now / Add without a photo) → sets `wardrobe_match` → the piece **reslots into Keep on re-render and the Pack CTA appears** (this is the intended add→keep→pack flow; the day-console rack mirrors it: unowned rows show `+ Add`, owned rows the Pack toggle). Card ids stay `tv-cap-${ci}` so `__tvSelectItem` / `__tvPackToggle` / `__tvSwap` are unchanged. **Leave behind only shows when the shortlist was 15+** (`data.shortlist_size`, stamped by `__tvSubmit`; falls back to owned-kept + cut for old saves) — cutting from a small shortlist read as nagging. The "The edit." summary spans the full container width (no `max-width`).
