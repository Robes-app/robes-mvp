...
- **Anchor Items (growth PRD epic 1 — Progressive Wardrobe Capture)**: the modal's anchor row shows a "Snap new" tile + wardrobe thumbnails; tap to anchor up to 3 owned pieces (`_tvBrief.anchors`). "Snap new" captures the modal state into `_tvBrief` (`_tvCaptureBrief`), runs the wardrobe add flow, and `_waAfterAdd` re-opens the modal via `__tvOpen({restore: true})` with the fresh piece pre-selected — modal state must survive the round trip. Server maps `anchorIds` → closet indexes, injects a NON-NEGOTIABLE prompt block (anchor must be packed + feature in ≥3 outfits), and the validation retry also fires when an anchor is dropped (`missingAnchors`).
