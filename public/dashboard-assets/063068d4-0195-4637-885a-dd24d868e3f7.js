...
const App = (function () {
  ...
  const UN = (id, w = 640) => (window.__resources && window.__resources['un_'+id]) || `https://images.unsplash.com/photo-${id}?w=${w}&q=80&auto=format&fit=crop`;
  let userName = 'Annie';
  let genTime = 2200;

  /* ── DATA ──────────────────────────────────────────────────────── */
  const WIMG = (n) => (window.__resources && window.__resources['w_'+n]) || `images/wimbledon/${n}.png`;
  ...
  const PIECE = {
    ...
    ways: [
      { eyebrow:'Off-duty', title:'With straight-leg jeans', img:UN('1485968579580-b6d095142e6e',900), pos:'50% 24%',
        ...
        suggestions:[
          'Button it over bare skin — no shirt, all structure.',
          'Ground it with high-rise indigo and a leather loafer.',
          'Finish with a gold hoop — sharp up top, easy below.',
        ],
        pieces:[
      ...
      { eyebrow:'After dark', title:'Over a taffeta skirt', img:UN('1485462537746-965f33f7f6a7',900), pos:'50% 32%',
        ...
        suggestions:[
          'Wear it as the top — tucked into a full taffeta midi.',
          'Let structure play against volume; a strappy sandal lifts it.',
          'A satin clutch is the only finish it needs.',
        ],
        pieces:[
      ...
      { eyebrow:'Quiet luxury', title:'With palazzo pants', img:UN('1490481651871-ab68de25d43d',900), pos:'50% 30%',
        ...
        suggestions:[
          'Build a clean column with wide cream palazzo.',
          'Belt the waist to define the line.',
          'Leather mules keep it quiet — and it reads evening.',
        ],
        pieces:[
  ...
  function renderPieces(host, pieces) {
    host.innerHTML = pieces.map((p, i) => {
      ...
      const actions = p.owned
        ...
        : `<button class="swap-cta" onclick="App.openSwap(${i})"><svg viewBox="0 0 24 24"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg> Swap</button>`;
    ...
    $('ways').innerHTML = PIECE.ways.map((w, i) => `
      <article class="way">
        ...
        <div class="way-body">
          ...
          <div class="way-outfit">
            <div class="way-outfit-label">The Outfit</div>
            <ul class="way-suggestions">
              ${(w.suggestions || [w.note]).map(s => `<li>${s}</li>`).join('')}
            </ul>
          </div>
            ...
            ${w.pieces.map((p, pi) => `
              <div class="wp${p.owned ? ' owned' : ''}">
                ...
                ${p.owned
                  ...
                  : `<button class="wp-swap" onclick="App.openWaySwap(${i},${pi})"><svg viewBox="0 0 24 24"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg>Swap</button>`}
              </div>`).join('')}
          </div>
        </div>
      </article>`).join('');
  ...
  /* the daily look — Stylist Directive: the 4-step curated layout */
  const LOOK_VIBES = {
    'Garden party':'A Dublin garden-party look.',
    'Dinner out':'A Dublin dinner, day to dark.',
    'Weekend errands':'A Dublin run-around look.',
    "The members' club":'A quiet Dublin members&rsquo;-club look.',
    'Travel day':'A Dublin departures look.',
  };
  // 6 wardrobe slots → exactly 4 narrative steps (by outfit index)
  const STEP_DEFS = [
    { n:1, title:'The Anchor',  idx:[0] },       // hero tailoring / outerwear
    { n:2, title:'The Canvas',  idx:[1,2] },     // core top + bottom base
    { n:3, title:'The Texture', idx:[5] },       // the layering / finishing piece
    { n:4, title:'The Accents', idx:[3,4] },     // footwear + bag
  ];
  const LOOK_SUMMARIES = {
    'Garden party': {
      hook:'Rain holds off until evening, so this leans into the lawn.',
      body:'<b class="ss-step">The Anchor</b> (%ANCHOR%) sets a polished, put-together line for the afternoon, balanced by <b class="ss-step">The Canvas</b> (%TOP% and %BOTTOM%) to keep your long lines intact and easy for a 21° day. <b class="ss-step">The Texture</b> (%TEXTURE%) adds that low-contrast, editorial depth.',
      tip:'For drinks tonight, drop the %BAG% for a clutch, push the blazer sleeves up, and you&rsquo;re set.',
    },
    'Dinner out': {
      hook:'A 12° walk in, then it warms up the moment you sit.',
      body:'<b class="ss-step">The Anchor</b> (%ANCHOR%) keeps the shoulder sharp on the way in, balanced by <b class="ss-step">The Canvas</b> (%TOP% and %BOTTOM%) for a long, deliberate line that holds all evening. <b class="ss-step">The Texture</b> (%TEXTURE%) is the one deliberate shine people remember.',
      tip:'The blazer comes off at the table — let the scarf and a sharper shoe carry the rest of the night.',
    },
    'Weekend errands': {
      hook:'Built to move, but never thrown on.',
      body:'<b class="ss-step">The Anchor</b> (%ANCHOR%) is the single structured note that reads considered, balanced by <b class="ss-step">The Canvas</b> (%TOP% and %BOTTOM%) to keep you comfortable and actually moving for an 18° day. <b class="ss-step">The Texture</b> (%TEXTURE%), tied to the bag, is effort where it shows.',
      tip:'When the day turns social, shrug off the blazer, swap the %BAG% for something smaller, and it reads evening.',
    },
    "The members' club": {
      hook:'A room that rewards restraint — so read it.',
      body:'<b class="ss-step">The Anchor</b> (%ANCHOR%) holds a clean cream column, fitted through the body, balanced by <b class="ss-step">The Canvas</b> (%TOP% and %BOTTOM%) to keep the line unbroken and quiet. <b class="ss-step">The Texture</b> (%TEXTURE%) is the single printed note, and the whole statement.',
      tip:'Into the evening, loosen the scarf and let the column soften — nothing else needs to change.',
    },
    'Travel day': {
      hook:'Variable skies, so this layers for all of it.',
      body:'<b class="ss-step">The Anchor</b> (%ANCHOR%) pulls soft layers together and survives the seat, balanced by <b class="ss-step">The Canvas</b> (%TOP% and %BOTTOM%) for a relaxed line that arrives uncreased. <b class="ss-step">The Texture</b> (%TEXTURE%) doubles as warmth on the plane and a finish on arrival.',
      tip:'Off the plane, swap the %SHOES% for a leather flat and the travel look becomes a dinner one.',
    },
  };
  function buildSummary() {
    const s = LOOK_SUMMARIES[occasion] || LOOK_SUMMARIES['Garden party'];
    const nm = i => (outfit[i] && W[outfit[i].cur]) ? W[outfit[i].cur].name : '';
    const fill = txt => txt
      .replaceAll('%ANCHOR%', `<i class="ss-piece">${nm(0)}</i>`)
      .replaceAll('%TOP%', `<i class="ss-piece">${nm(1)}</i>`)
      .replaceAll('%BOTTOM%', `<i class="ss-piece">${nm(2)}</i>`)
      .replaceAll('%SHOES%', `<i class="ss-piece">${nm(3)}</i>`)
      .replaceAll('%BAG%', `<i class="ss-piece">${nm(4)}</i>`)
      .replaceAll('%TEXTURE%', `<i class="ss-piece">${nm(5)}</i>`);
    return `<div class="stylist-summary">
      <div class="ss-label">Stylist Summary</div>
      <p class="ss-body">${fill(s.hook)} ${fill(s.body)}</p>
      <p class="ss-tip"><span class="ss-tip-label">Transition tip</span>${fill(s.tip)}</p>
    </div>`;
  }

  function renderOutfit() {
    const fw = FRAMEWORK[occasion] || FRAMEWORK['Garden party'];
    const first = (userName || 'Annie').split(/\s+/)[0];
    ...
    const swapGlyph = '<svg class="step-swap-ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>';

    // THE 4-STEP CURATED LAYOUT — Anchor · Canvas · Texture · Accents (with one summary on top)
    $('look-board').innerHTML = buildSummary() + STEP_DEFS.map(def => {
      const items = def.idx.filter(i => outfit[i]).map(i => ({ i, s: outfit[i], it: W[outfit[i].cur] }));
      if (!items.length) return '';

      const imgs = items.map(({ it, i }, k) =>
        `<div class="step-img">
          <img src="${it.img}" style="object-position:${it.pos}" alt="">
          ${k === 0 ? `<span class="step-badge">${def.n}</span>` : ''}
          <button class="step-img-swap" title="Swap ${it.name} from your wardrobe" onclick="App.openLookSwap(${i})">${swapGlyph}</button>
        </div>`).join('');

      const pieceLines = items.map(({ it, i }) =>
        `<div class="step-piece">
          <span class="sp-id"><strong>${it.name}</strong><span class="sp-brand">${it.brand}</span></span>
          <button class="step-swap" onclick="App.openLookSwap(${i})" title="Swap from your wardrobe">${swapGlyph}Swap</button>
        </div>`).join('');

      return `<article class="step">
        <div class="step-imgs" data-n="${items.length}">${imgs}</div>
        <div class="step-body">
          <div class="step-eyebrow">Step ${def.n}</div>
          <h3 class="step-title">${def.title}</h3>
          <div class="step-pieces">${pieceLines}</div>
        </div>
      </article>`;
    }).join('');
  }
  ...
  /* wardrobe grid */
  const WFILTERS = ['All','Outerwear','Tops','Bottoms','Shoes','Accessories','Dresses','Bags','Swimwear'];
  let wfilter = 'All';
  function renderWardrobe() {
    ...
    $('wg-grid').innerHTML = items.map(it => {
      const visual = it.img
        ? `<img src="${it.img}" style="object-position:${it.pos || '50% 50%'}" alt="">`
        : `<div class="wg-swatch-fill" style="background:${it.colour || '#E7E0CF'}"><span>${(it.name || '?').trim().charAt(0).toUpperCase()}</span></div>`;
      return `<div class="wg-item">
        <div class="wg-img-wrap">
          ${visual}
          ${it.wears === 0 ? '<span class="wg-owned-badge">Never worn</span>' : ''}
        </div>
        <div class="wg-info"><div class="wg-name">${it.name}</div><div class="wg-metar">${it.brand ? it.brand + ' · ' : ''}${it.wears === 0 ? 'New' : it.wears + '× worn'}</div></div>
      </div>`;
    }).join('') +
      `<div class="wg-item wg-add" onclick="WA.open()"><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>`;
    if ($('wg-count')) $('wg-count').textContent = `${WORDER.length} pieces`;
    const navCount = document.querySelector('.nav-wbtn-count');
    if (navCount) navCount.textContent = WORDER.length;
  }
  function addPiece(d) {
    const id = 'u' + Date.now();
    W[id] = { name: d.label, brand: d.brand || '', group: d.category || 'Accessories',
              wears: 0, img: d.photo || '', pos: '50% 50%', colour: d.colour || '', notes: d.notes || '' };
    WORDER.push(id);
    if (wfilter !== 'All' && wfilter !== W[id].group) wfilter = 'All';
    renderWardrobe();
    toast(`${d.label} added to your wardrobe`);
    return id;
  }
  ...
  /* ── SWAP (unowned piece) — capture wardrobe data, or shop ───────── */
  let swapIdx = null;
  let swapMode = 'board';   // 'board' = moodboard piece · 'look' = daily-look slot
  let lookSwapIdx = null;
  // toggle the overlay chrome between moodboard (shop) and look (wardrobe-only) modes
  function setSwapChrome(mode) {
    const shopBtn = document.querySelector('.swap-act-shop');
    const note = $('swap-shop-note');
    const ey = document.querySelector('#swap-overlay .swap-ey');
    const label = document.querySelector('#swap-overlay .swap-lib-label');
    const look = mode === 'look';
    if (shopBtn) shopBtn.style.display = look ? 'none' : '';
    if (note) note.style.display = look ? 'none' : '';
    if (ey) ey.textContent = look ? 'Pull from your wardrobe' : 'Swap this piece';
    if (label) label.textContent = look ? 'Swap in another piece you own' : 'From your wardrobe';
  }
  // open the swap overlay against a slot in today's look
  function openLookSwap(i) {
    const s = outfit[i];
    const cur = s && W[s.cur];
    if (!cur) return;
    swapMode = 'look';
    lookSwapIdx = i;
    const ov = $('swap-overlay');
    if (!ov) { toast(`Finding an alternative to ${cur.name}`); return; }
    if ($('swap-piece-name')) $('swap-piece-name').textContent = cur.name;
    if ($('swap-piece-brand')) $('swap-piece-brand').textContent = cur.brand || '';
    setSwapChrome('look');
    renderSwapLib(cur.group, s.cur);
    ov.classList.add('open');
  }
  // apply a wardrobe piece into the current look slot
  function applyLookSwap(id) {
    const i = lookSwapIdx;
    if (i == null || !outfit[i] || !W[id]) return;
    const s = outfit[i];
    if (s.cur !== id) {
      const old = s.cur;
      s.cur = id;
      s.alts = [old, ...s.alts.filter(x => x !== id && x !== old)];
    }
    renderOutfit();
    toast(`Styled with your ${W[id].name}`);
  }
  // open the swap overlay against a piece inside a Style-a-piece "way"
  let waySwap = null;   // { w, p }
  function openWaySwap(wi, pi) {
    const w = PIECE.ways[wi];
    const p = w && w.pieces && w.pieces[pi];
    if (!p) return;
    swapMode = 'way';
    waySwap = { w: wi, p: pi };
    const ov = $('swap-overlay');
    if (!ov) { toast(`Finding an alternative to ${p.name}`); return; }
    if ($('swap-piece-name')) $('swap-piece-name').textContent = p.name;
    if ($('swap-piece-brand')) $('swap-piece-brand').textContent = p.brand || '';
    if ($('swap-shop-note')) $('swap-shop-note').textContent = p.retailer
      ? `Opens ${p.retailer}${p.price ? ' \u00b7 ' + p.price : ''}` : 'Opens the retailer in a new tab.';
    setSwapChrome('board');
    renderSwapLib(p.group);
    ov.classList.add('open');
  }
  function applyWaySwap(src) {
    const ws = waySwap;
    if (!ws) return;
    const w = PIECE.ways[ws.w];
    const p = w && w.pieces && w.pieces[ws.p];
    if (!p) return;
    p.owned = true;
    if (src) { p.name = src.name; p.brand = src.brand || p.brand; if (src.img) p.img = src.img; }
    renderPiece();
  }
  function restatRail() {
    const b = BOARDS[currentBoard];
    if (!b || !b.pieces) return;
    const yours = b.pieces.filter(p => p.owned).length;
    const shop = b.pieces.length - yours;
    if ($('mb-rail-stat')) $('mb-rail-stat').innerHTML = `<span class="s">${yours}</span> yours \u00b7 <span class="r">${shop}</span> to shop`;
    if ($('mb-rail-sub')) $('mb-rail-sub').textContent = `${b.pieces.length} pieces \u00b7 ${yours} from your wardrobe`;
  }
  function openSwap(i) {
    const b = BOARDS[currentBoard];
    const p = b && b.pieces && b.pieces[i];
    if (!p) return;
    swapIdx = i;
    swapMode = 'board';
    const ov = $('swap-overlay');
    if (!ov) { toast(`Finding an alternative to ${p.name}`); return; }   // graceful fallback where the modal isn't present
    if ($('swap-piece-name')) $('swap-piece-name').textContent = p.name;
    if ($('swap-piece-brand')) $('swap-piece-brand').textContent = p.brand || '';
    if ($('swap-shop-note')) $('swap-shop-note').textContent = p.retailer
      ? `Opens ${p.retailer}${p.price ? ' \u00b7 ' + p.price : ''}` : 'Opens the retailer in a new tab.';
  ...
  function closeSwap() { const ov = $('swap-overlay'); if (ov) ov.classList.remove('open'); }
  /* legacy single-tap own-capture (used by the original dashboard's swap modal) */
  function swapAddYours() {
    const b = BOARDS[currentBoard];
    const p = b && b.pieces && b.pieces[swapIdx];
    closeSwap();
    if (!p) return;
    applySwapOwned(p);
    toast(`${p.name} added to your wardrobe`);
  }
  function renderSwapLib(group, currentId) {
    const host = $('swap-lib');
    if (!host) return;
    let ids = WORDER.filter(id => W[id] && id !== currentId);
    if (group) ids.sort((a, b) => (W[b].group === group) - (W[a].group === group));   // relevant category first, full wardrobe still shown
    host.innerHTML = ids.map(id => {
      const it = W[id];
      const vis = it.img
        ? `<img src="${it.img}" style="object-position:${it.pos || '50% 50%'}" alt="">`
        : `<span class="swap-lib-sw" style="background:${it.colour || '#E7E0CF'}">${(it.name || '?').trim().charAt(0).toUpperCase()}</span>`;
      return `<button class="swap-lib-item" title="${it.name}" onclick="App.swapPickFromLibrary('${id}')">
        <span class="swap-lib-thumb">${vis}</span>
        <span class="swap-lib-name">${it.name}</span>
      </button>`;
    }).join('');
  }
  function applySwapOwned(p, src) {
    p.owned = true; p.wears = p.wears || 'New';   // capture into the wardrobe even without a purchase
    if (src) { p.name = src.name; p.brand = src.brand || p.brand; if (src.img) p.img = src.img; }
    const b = BOARDS[currentBoard];
    if (b && b.pieces) renderPieces($('mb-rail-pieces'), b.pieces);
    restatRail();
  }
  function swapPickFromLibrary(id) {
    if (swapMode === 'look') { closeSwap(); applyLookSwap(id); return; }
    if (swapMode === 'way') { closeSwap(); if (W[id]) { applyWaySwap(W[id]); toast(`Styled with your ${W[id].name}`); } return; }
    const b = BOARDS[currentBoard];
    const p = b && b.pieces && b.pieces[swapIdx];
    closeSwap();
    if (!p || !W[id]) return;
    applySwapOwned(p, W[id]);
    toast(`Styled with your ${W[id].name}`);
  }
  let swapPending = null;
  function swapUploadNew() {
    if (swapMode === 'way') {
      const ws = waySwap;
      closeSwap();
      const finish = newId => { waySwap = ws; if (W[newId]) { applyWaySwap(W[newId]); toast(`Styled with your ${W[newId].name}`); } };
      if (window.WA && typeof WA.openFor === 'function') WA.openFor(finish);
      else if (window.WA) WA.open();
      return;
    }
    if (swapMode === 'look') {
      const i = lookSwapIdx;
      closeSwap();
      const finish = newId => { lookSwapIdx = i; applyLookSwap(newId); };
      if (window.WA && typeof WA.openFor === 'function') WA.openFor(finish);
      else if (window.WA) WA.open();
      return;
    }
    swapPending = { board: currentBoard, idx: swapIdx };
    closeSwap();
    if (window.WA && typeof WA.openFor === 'function') WA.openFor(resolveSwapWithPiece);
    else if (window.WA) WA.open();
  }
  function resolveSwapWithPiece(id) {
    const ps = swapPending; swapPending = null;
    if (!ps || !W[id]) return;
    const b = BOARDS[ps.board];
    const p = b && b.pieces && b.pieces[ps.idx];
    if (!p) return;
    applySwapOwned(p, W[id]);
    toast(`Styled with your ${W[id].name}`);
  }
  function swapShop() {
    if (swapMode === 'look') { closeSwap(); return; }
    if (swapMode === 'way') {
      const ws = waySwap, w = ws && PIECE.ways[ws.w], p = w && w.pieces[ws.p];
      closeSwap();
      if (p) toast(`Opening ${p.brand}${p.retailer ? ' at ' + p.retailer : ''}`);
      return;
    }
    const b = BOARDS[currentBoard];
    const p = b && b.pieces && b.pieces[swapIdx];
    closeSwap();
    if (p) toast(`Opening ${p.brand} at ${p.retailer}`);
  }
  let toastT;
  function toast(msg) {
  ...
  function replayOnboarding() {
    closeAvatarMenu();
    if (!window.RobesOnboarding) return;   // onboarding not loaded in this build
  ...
  function init() {
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape') { ['pack-sheet','share-sheet'].forEach(closeSheet); closeRename(); closeSwap(); closeAvatarMenu(); closeAddMenu(); }
    });
    ...
    if (window.RobesOnboarding && /[?&]onboard=1/.test(location.search)) {
  ...
  return {
    ...
    showWardrobe, filterWardrobe, addPiece, openPack, openShare, closeSheet, changeLimit,
    copyLink, openRename, closeRename, saveRename, toast,
    openSwap, closeSwap, swapShop, swapAddYours, openLookSwap, openWaySwap,
    swapPickFromLibrary, swapUploadNew,
