...
## ⚠ THIS BRANCH: P0 simplification (`claude/beta-deployment-setup-wdvxb6`)
This branch duplicates `signup-flow` and layers the **P0 simplification** (tech handoff 2026-07-11: wardrobe-first utility engine) on top, for live testing on beta.byrobes.com (point the Railway staging service's source branch at this branch; point it back at `signup-flow` to restore the current beta). Deltas vs `signup-flow`:
