...
- **The day console** (`_tvPaintConsole` → `#tv-look-panel` + `#tv-rackwrap`) emulates the Daily experience: LEFT — dark "The Look" moodboard panel with a **Day/Evening flick** (`__tvSetOcc`), the slot's `how` line as the mood quote, the trip's editorial hero shot as the wide "The mood" tile (frame 0), then a tile per formula piece (owned ✓ / dashed "Add" for unowned; tap = swap), fabrics + palette + "N of M already yours", and "The read" meter beneath (packed count of the look's pieces + a gap/pack verdict). RIGHT — "The outfit · Day n" (eyebrow renamed from "The rack · Day n" for clarity): each formula entry as a card (role label, "× N looks", name, "✓ In your wardrobe" or "Worth adding" + retailer · price, the formula `note` in italic, the `.tvm-packbox` Pack/Packed checkbox + Swap), headed by "✎ The real plan" (opens `__tvEditDay(_tvActiveDay)`) and "Pack this look".
