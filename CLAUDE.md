...
- **Refine drawer** (`#rb-refine`, toggled by a right-aligned `Refine` pill with active-count badge): category pills stay primary; season (standalone Spring/Summer/Autumn/Winter/Year-round multi-select), colour swatches (only colours she owns, from `_ALL_SWATCHES` — now at wardrobe-wiring scope), silhouette/fit chips (union of `item_dna.structural_dna.silhouette_fit`), worn/never segmented control, brand dropdown, live "Show N pieces" + Clear all. Untagged pieces count as Year-round in every season filter.
...
- **Refine**: `_waMatchRefine(it)` + `_waFilteredItems()` drive `_waRender`; `_waRefineRender()` rebuilds the drawer per interaction; `_waRefinePillSync()` keeps the pill badge. Category pill `.active` toggles exclude `.rb-refine-pill` AND `.wg-pack-pill` (both handlers).
