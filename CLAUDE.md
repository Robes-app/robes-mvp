...
- **Prompt pills stripped** — no chip row under the concierge box; the bundle's typewriter rotating placeholder (its examples in `dashboard-assets/144529f6….js` `PROMPT_EXAMPLES`, now incl. a weekly line) is the sole intent guide. The photo/"From wardrobe" attach row is now **always visible** (`_cbShowPhotoZone` never hides the zone, only clears state). `_cbSetIntent` survives as the concierge-card scaffold injector (Daily / Weekly cards).
...
- **Post-add fork modal** (`window.__rbAddFork`) — after every successful non-edit wardrobe add (skipped when `_waAfterAdd` was armed by a swap/snap-mine flow; fires once at the END of a batch): "Style it 3 ways" (→ `_cbStyleSubmit` with the piece's photo re-fetched as dataURL) or "Build today's look around it" (→ `__dlSubmit`).
