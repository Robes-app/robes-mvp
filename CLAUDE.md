...
| `public/onboarding.html` | First-time-user onboarding flow — splash/intro/name + 3 steps (icons/colour/first piece) + done, saves to `profiles` |
...
- `/onboarding` — first-time-user onboarding flow (standalone `onboarding.html`, 3 steps: icons/colour/first piece — silhouette moved to Style Notes, see its section below)
...
**Silhouette completion is now here, not in onboarding.** Since onboarding dropped the body-photo step, a quiet **below-the-fold dashboard card (`_rbSilPrompt` in `__robes_personalize`)** invites her to finish it: eyebrow "Style notes · one chapter left", CTA "Complete my silhouette →" → `/stylenotes#silhouette`. Gated on `window.__robes_profile.style_dna.silhouette_proportions` being absent, dismissible per-user (`rb_sil_prompt_off__<uid>` in localStorage), inserted before the concierge service cards (delayed 1200ms past `_rbApplyLayout` so the reshuffle can't displace it).
...
`public/onboarding.html` is a standalone protected page (vanilla, no framework) ported from the Claude Design "Robes Onboarding Flow v2" bundle. One state machine (`st.stage`): dark splash (auto-advances 2s) → intro → name → 3 cream working steps (`STEPS = ['icons','colour','keypiece']`) → dark done screen. **Silhouette is deliberately NOT in onboarding** (optimization 4.7): a full-length body photo minutes into a first session was the likeliest bail point, so it moved to Style Notes and a quiet dashboard prompt (`_rbSilPrompt`) invites her to finish it later. Colour stays in the flow because it lands before the first piece and feeds the styling prefire's DNA. Step count, progress bar and done-screen dots all derive from `STEPS.length`. All copy/tokens come from the mockup — Cormorant + Inter, `--primary` ink dark arc, cream `#FAF8F5` steps, 2px radii, hairline borders.
...
- **Skip discipline**: on the colour photo step the filled "Continue" primary is hidden until a reading exists (`syncFooter()` — the only footer action pre-read is the light "Skip for now"). On step 03, tapping Skip once with no piece filed arms a two-tap confirm ("Skip anyway — Robes styles only what you add"); a second tap (or Skip when a piece is already filed) goes to done.
...
- **Chip = explicit override.** The three chips (Style a key piece / Dress me today / Create a moodboard) still exist as prompt scaffolds and manual overrides — a deliberate deviation from the PRD's "remove the pills": they double as the clarifying affordance. **Chip templates are short** (optimization 4.7 — the long ones bled off-screen on mobile): "Style my [cream blazer] three ways", "An outfit for [Sunday brunch] with my [trench coat]", "A moodboard for [a weekend in Ibiza]". On inject the caret lands on the first `[bracket]` for quick editing.
...
- **Bracket guard (`_cbSubmit`)**: a submit whose text still contains `[…]` never fires the API — it selects the bracket and toasts "Make it yours — swap the highlighted part for your own piece."
...
- **The day console** (`_tvPaintConsole` → `#tv-look-panel` + `#tv-rackwrap`) **mirrors the Daily console** (consistency pass 2026-07-10): LEFT — "The Look" moodboard panel headed **"The look · Day n · N pieces"** (same register as Daily's "The look · N pieces"), with a **Day/Evening flick** (`__tvSetOcc`), the slot's `how` line as the mood quote, the trip's editorial hero shot as the wide "The mood" tile (frame 0), then a tile per formula piece (owned ✓ / dashed "Add" for unowned; tap = swap, **hover flick arrows** `.tnav` → `__tvFlip`), fabrics + palette + "N of M already yours", and "The read" meter beneath (packed count of the look's pieces + a gap/pack verdict). RIGHT — eyebrow **"The rack · Day n"** (was the prescriptive "The outfit · Day n"): each formula entry as a card (garment-slot label, "× N looks", name, "✓ In your wardrobe" or "Worth adding" + retailer · price, the formula `note` in italic, an "i / n" option count on the viewport, then a footer with the **flick-through cluster** (arrows + dots, exactly the Daily rack's) on the left and the `.tvm-packbox` Pack/Packed checkbox (or `+ Add`) + Swap on the right), headed by "✎ The real plan" (opens `__tvEditDay(_tvActiveDay)`) and "Pack this look".
