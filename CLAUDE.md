...
- `/api/stylenotes/analyse` resilience: `thinkingConfig: { thinkingBudget: 0 }` + `maxOutputTokens` 2048 (the observation/reasoning fields are the chain-of-thought — real thinking stays off so the JSON never truncates). Attempt 1 sends the `responseSchema`; on any Gemini error attempt 2 retries **without** the schema (JSON mode + the prompt's field spec — renderers validate shape anyway). Both failing → 502 `{ error: 'analysis_failed', reason }`; the client logs `reason` to the console and Railway logs carry `[stylenotes/analyse] attempt N failed`.
