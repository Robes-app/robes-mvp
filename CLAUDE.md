...
- Travel Edit: `/api/travel` + `__tvRenderResult` — **curatorial** capsule packing & lookbook generator, now **plan-first** (travel-edit UX 2026-07-10): brief modal with full-wardrobe shortlist browser → **day planner modal** (accept/edit/skip each day's plan before any looks generate) → result page split by two large tabs (The Edit = Keep / Worth Adding / Leave Behind + packed checklist; Outfits = day strip + a Daily-style Look/Rack console with Day/Evening flick **and per-piece flick-through**), 1:3 interactive multiplier, reactive day restyle via `/api/travel/day`, PDF export, sticky payoff bar, see its section below
...
- **Day planner (`_tvShowPlanModal`, `#tv-plan-modal`) — step 2 of 2 (travel-edit UX 2026-07-10)**: the flow is Trip > pick pieces > **plan your days** > outfits — she accepts, edits or clears each day BEFORE any looks are generated, so the itinerary is hers, not an LLM guess she corrects after the fact. One input per trip day (`_tvTripDays` mirrors the server's day maths: min(10, inclusive span), 7 when dates don't parse), labelled "Day N · Fri 12 Jul", rotating placeholder suggestions, a smart-chip row that fills the focused input (`__tvPlanChip`/`_tvPlanFocus`), "← Your pieces" back to the brief modal (`__tvPlanBack` → `__tvOpen({restore: true})` — `_tvBrief.plan` survives that round trip and the snap-new loop via `_tvCaptureBrief`), primary "Create my outfits · N days →", and a "Skip — let Robes plan the days" escape (`__tvPlanGo(true)`). A blank day means "Robes reads the trip". Server: `dayPlan` (array of strings aligned to trip days) is sanitized and injected as an **authoritative itinerary block** — a planned day is dressed for exactly that plan and its `day_label` derives from it — and `days[i].user_activity` is stamped on planned days so the day strip shows "· your plan" from the first render and `✎ The real plan` prefills. `/api/travel/day` (reactive restyle) is unchanged as the post-generation correction path.
