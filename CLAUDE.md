...
- Dashboard v2 layout (wardrobe-first): Styling Concierge → Wardrobe tracker (+ onboarding "Your piece, styled" card) → Moodboards + Lookbook rows → Concierge service cards (all live)
...
- Travel Edit: `/api/travel` + `__tvRenderResult` — **curatorial** capsule packing & lookbook generator (brief modal with full-wardrobe shortlist browser → Keep / Worth Adding / Leave Behind + day-by-day Day/Evening lookbook with 4-step formulas, 1:3 interactive multiplier, packed checklist, reactive day restyle via `/api/travel/day`, PDF export), see its section below
...
- Step 04 First piece — wardrobe capture with **no exit gate** (wardrobe-first PRD): photo panel (same `.cap` chrome as steps 02/03) + optional single-line stylist note; the "What Robes files" ledger reads The piece / Category / Brand / Colour / Cut. A photo auto-runs `catalogue()` — the same `/api/wardrobe/analyse` + `/api/wardrobe/upload` pair as the dashboard add flow (one retry on `analysisFailed`/missing label), then: row inserted into `wardrobe_items` via `sbClient` (background, promise kept in `st.piece.savePromise`), `prefireStyle()` fires `/api/style` immediately (result → `sessionStorage.rb_onboard_styled`), `writeHandoff()` writes `sessionStorage.rb_onboard_piece = {prompt, photo, cataloged}` (prompt = note or analysed label; `cataloged` stamped async once the insert settles, 3s cap; photo dropped on quota errors), scan-line animation → extracted tags pop staggered (`kpTagPop`) → **auto-advance to done** after the reveal beat. The confirmation modal and full-screen "Style it three ways" step are REMOVED from the critical path — the only pause is a user focused in the note input (the "Continue to your dashboard" CTA carries her forward, re-running `writeHandoff()` so a late note rides along). `noItemDetected` clears the photo with an inline error; a hard analysis failure keeps the photo so the CTA can be retried. No photo → nudge; Skip still goes to done.
- Done screen — sets `onboarded_at` + `sessionStorage.rb_onboarded`, CTA "Enter your dashboard" → `/dashboard`.

**Dashboard handoff — the "Your piece, styled" card**: `_rbOnboardHandoff` IIFE at the end of `__robes_personalize` reads + clears `rb_onboard_piece`/`rb_onboard_styled` and renders an **inline card** (`#rb-styled`) glued under the wardrobe tracker — the relocated wow moment, NOT a full-screen overlay. Loading state (3 pulsing tiles) → ready state (3 look tiles + titles, images patched in by its own light poller on `data.jobId`); "See the full looks →" opens `window.__kpRenderResult(data, prompt, {intent:'style'})` (which runs its own polling + lookbook save). Fresh prefire (<10 min) renders ready instantly; otherwise `quietStyle()` re-fires `/api/style` with no overlay; a hard failure paints a retry card that falls back to the classic `_cbStyleSubmit` overlay flow. The handoff's background wardrobe persist only runs when the payload lacks `cataloged: true` (legacy payloads or an onboarding insert that failed) — step 04 normally saves the item itself, so this guard is what prevents duplicate wardrobe rows.
...
- **Three result sections replace the tier-grouped capsule**: Keep (owned cards with reason in italic), Worth adding (unowned cards with bridge + retailer · price), Leave behind (compact muted rows, greyscale-ish thumb + reason, `tv-noprint`). Card ids stay `tv-cap-${ci}` (index into `data.capsule`) so `__tvSelectItem` / `__tvPackToggle` / `__tvSwap` work unchanged; tiers still exist in the schema/prompt as internal architecture. A day-restyle `new_item` or a quick-own/swap moves cards between Keep and Worth adding automatically on re-render.
- **"Packed It" checklist (epic 3)**: every capsule card carries a Packed checkbox (`__tvPackToggle`, surgical DOM update — no re-render, so scroll + 1:3 selection survive) feeding the `#tv-pack-progress` bar next to "The edit" heading ("Packed n of m" → "Suitcase closed — bon voyage ✈"). Packing a **curated** (unowned) piece opens `#tv-own-modal` — the conversion moment: "Snap it now" (arms `_waAfterAdd` → `__tvSwapApply`) or "Add without a photo" (`__tvQuickOwn`: direct `wardrobe_items` insert from the capsule metadata, `Swim`→`Swimwear` category mapping, `_waLoad()` refresh so tracker/counts update, then sets `wardrobe_match` and re-renders preserving scroll). `packed` flags live on capsule items inside `tvData`, so they persist and restore on reopen; every mutation patches the saved entry via `_tvPatchSaved`.
...
`section.tracker#wtrk` in dashboard.html — headline "The more you catalogue, *the more Robes can style.*" over a sage card. `_waSyncCounts()` fills: `#wtrk-num` (n / 15), `#wtrk-kicker` + `#wtrk-copy` (milestone copy via `_wtrkCopy(n)`: Start here / Keep going / Nearly there / Wardrobe complete — the body always leads with the bold count, e.g. "**1 of 15 pieces catalogued** — 14 more and Robes can dress you head to toe. It styles only from what you own."), `#wtrk-fill` (progress bar), and `#wtrk-items` — the first 12 catalogued pieces as thumbnail tiles (click → `window.__wtrkEdit(id)` → edit modal), a "+N more" tile → `App.showWardrobe()`, and a dashed Add tile. `#wtrk-cta` ("Add your first piece" / "Add pieces") and the `#wtrk-snap` "Catalogue what you're wearing now" fastest-add both open the wardrobe add modal directly — they do NOT navigate to `/wardrobe`. The section keeps the `.tracker` class (it's `_rbApplyLayout`'s anchor); `_rbApplyLayout` moves it above the moodboards/lookbook rows so the wardrobe leads the page, and re-glues `#rb-styled` (the onboarding card) after it.
