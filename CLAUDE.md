...
- **Amplified add entry**: the bare `+` grid card became a weighted card (`.rb-add-card`: + circle, Cormorant "Add a piece", "Photograph · Link · Receipt") opening a method chooser (`__waAddChooser`) — Photograph is live (→ `WA.open()`), Paste a link + Import a receipt are dimmed Coming-soon doors (→ `KP.comingSoon`). A persistent header CTA (`#rb-wg-cta-head`, "+ Add a piece" / "+ Save a piece" on wishlist) sits beside the count so adding never needs a scroll. Phase 2 = making the link + receipt doors real (copy-link import, receipt OCR).
...
- **Layout (beta feedback 2026-07-10)**: `.wardrobe-panel` is constrained to the app container (`max-width:1148px;margin:0 auto`, matching `.dash`) instead of full-width; under 640px `#wg-filters` becomes a single horizontal scroll row (nowrap, hidden scrollbar, `flex-shrink:0` pills) instead of wrapping into a cluttered block.
...
- **Setup** (`_waV2Setup` IIFE): injects `#rb-wa2-style` + into the static panel: `#rb-wsub` sub-tabs before `.wg-header`, `.rb-wg-actions` (persistent `#rb-wg-cta-head` CTA + relocated `#wg-count`) into the header, `#rb-hero` before `#wg-filters`, `#rb-refine` after it, `#rb-wl-grid` after `#wg-grid`. All view flips go through `_waV2Sync()` (called at the end of every `_waRender`); `__waSetView(v)` also sets crumb + `_rbNav('/wardrobe'|'/wishlist')`. **`#rb-wl-grid` needs `display:grid` inline when shown** — its stylesheet default is `none`, so clearing the inline style hides it.
