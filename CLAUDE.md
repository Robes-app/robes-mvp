...
The analyse endpoint is **deterministic**: Gemini (`gemini-2.5-flash`) extracts measurable primitives only — colour: undertone (Warm/Cool/Neutral-Warm/Neutral-Cool), contrast (Low/Medium/High/Extremely High), chroma, lightness, skin/hair/eye hexes, `low_confidence`; silhouette: shoulder/waist/hip ratios + `loose_clothing`. Everything the user sees comes from `style_dna.js`:
- **Extraction runs at `temperature: 0`** (the default 1.0 was sampling different undertone/contrast/chroma enums for the same photo, cascading into a different season on each retry — this is why colour results looked inconsistent). The colour prompt enforces a measurement protocol: neutralise ambient colour cast against the sclera/teeth first, ignore background/clothing/makeup, weigh vein colour + cast-corrected skin for undertone. Accuracy ceiling is still the model perceiving colour semantically not colorimetrically — the parked next step is true PRD §4.1 colorimetry (Gemini locates skin/hair/iris/sclera regions, server samples pixels via `sharp`, LAB + sclera white-balance decide undertone/contrast).
...
- `classifyBody()` — PRD §5.1 width-ratio thresholds → one of 5 body archetypes (`BODIES` catalog from the Master Archetype Registry: 4 dresses + notes, 4 necklines, 4 display tips, 4 full styling maxims). Loose clothing + flat ratios → Rectangle fallback with `fallback_applied: true`.
