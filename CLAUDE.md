...
- **Client render** (`__dlRenderResult(data, promptText, opts)` in dashboard-personalize.js, page `#dl-result-page`, z-index 40 under the nav; classed CSS injected as `_DL_CSS` into `#dl-style` on every render): masthead = short serif `headline` H1 ("A Dublin Sunday brunch look.") with occasion + weather chips on the right; then the console grid `356px | 1fr` — the rack deliberately gets the larger share.
...
- **Three result sections form "The edit."** (below the console): Keep (owned cards with reason in italic), Worth adding (unowned cards with bridge + retailer · price), Leave behind (compact muted rows, greyscale-ish thumb + reason, `tv-noprint`). Card ids stay `tv-cap-${ci}` (index into `data.capsule`) so `__tvSelectItem` / `__tvPackToggle` / `__tvSwap` work unchanged; tiers still exist in the schema/prompt as internal architecture. A day-restyle `new_item` or a quick-own/swap moves cards between Keep and Worth adding automatically on re-render.
...
- **Client render (`__tvRenderResult(data, opts)`, `#tv-result-page`, z-index 40; classed CSS `_TV_CSS` → `#tv-style`)** — calendar-first (UX amendments 2026-07): masthead = short serif `headline` H1 ("A Biarritz week of sun & surf.") + packed-progress cluster on the right, then chips (destination · dates ✈, temp/condition, italic location-vibe tag).
...
The rail's eyebrow in dashboard.html reads **"The rack"** (renamed from "The look", UX amendments 2026-07 — the clothing list is called The Rack on every surface). Each row in `#mb-rail-pieces`: 44×44px thumbnail (wardrobe photo or `#EDE8E0` placeholder) + name/brand + pills + CTA. Padding `10px 20px` to match bundle's 20px horizontal standard.
