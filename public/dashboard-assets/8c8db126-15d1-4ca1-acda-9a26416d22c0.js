/* ROBES — Dashboard Tweaks: host protocol + live application */
(function () {
  const DEFAULTS = { accent:'#8E7077', name:'Annie', genTime:2200, weather:true, prompt:'left', density:'comfortable' };
  const ACCENTS = [
  ...
  function apply() {
    ...
    if (typeof App !== 'undefined' && App.setGenTime) App.setGenTime(t.genTime);
    if (typeof App !== 'undefined' && App.setName && t.name) App.setName(t.name);
    const nw = document.getElementById('nav-weather'); if (nw) nw.style.display = t.weather ? '' : 'none';
    document.body.dataset.prompt = t.prompt;
  ...
  function seg(id, val, opts) {
    return `<div class="tw-seg" id="${id}">` + opts.map(([v, label]) =>
      `<button class="tw-segbtn${val === v ? ' on' : ''}" data-v="${v}">${label}</button>`).join('') + `</div>`;
  }

  function render() {
    panel.innerHTML = `
      <div class="tw-hd"><span class="tw-title">Tweaks</span><button class="tw-close" id="tw-close">&times;</button></div>
      <div class="tw-row">
        <span class="tw-label">Accent tone</span>
      ...
      <div class="tw-row">
        <span class="tw-label">Prompt layout</span>
        ${seg('tw-prompt', t.prompt, [['left','Left'],['center','Centered']])}
      </div>
      <div class="tw-row">
        <span class="tw-label">Density</span>
        ${seg('tw-density', t.density, [['comfortable','Comfortable'],['compact','Compact']])}
      </div>
      ...
      ${window.RobesOnboarding ? '<div class="tw-divider"></div><button class="tw-btn" id="tw-replay">Replay onboarding</button>' : ''}`;
    wire();
  }
  function wire() {
    ...
    panel.querySelector('#tw-prompt').querySelectorAll('[data-v]').forEach(b => b.addEventListener('click', () => {
      t.prompt = b.dataset.v; apply(); persist(); render();
    }));
    panel.querySelector('#tw-density').querySelectorAll('[data-v]').forEach(b => b.addEventListener('click', () => {
      t.density = b.dataset.v; apply(); persist(); render();
    }));
    ...
    const replay = panel.querySelector('#tw-replay');
    if (replay) replay.addEventListener('click', () => { if (typeof App !== 'undefined') App.replayOnboarding(); });
