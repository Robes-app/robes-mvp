...
| 3 | Robes builds from yours at five + the ladder retune — the ladder retune built 2026-09-18, **3.1 and 3.3 built 2026-09-21** (Annie: "lets build 3.1 & 3.3 now") | 3 | no | M | 1 |
...
> **Built in two passes.** 2026-09-18: the ladder retune (3.2) with Annie's copy — caps **Builds a daily look · Plans a week of outfits · Knows your taste** (keys `daily` / `weekly` / `styleNotes`, `pos` 25 / 50 / 78; not the "Builds from yours · Packs your trips" pair below), the concierge band's header meter rendering that ladder (the shared `_msBarHtml`, ticks + capability columns, no transition) in place of the linear fill, the Daily and Weekly card captions reading their rung. 2026-09-21 (Annie: "lets build 3.1 & 3.3 now"): **3.1** — `_LK_ROBES_AT` reads `_MS_UNLOCKS[0].at` (the `daily` rung, five; there is no `robes` key — one ladder), the composer pill beside Save (`.rb-lk-robesdoor`, a `.rb-pill`, the two wordings, never on a build already standing or a kp / prompted draft), the `five` and `robes` next-line rules between `finish` and the diary rules, `robes_build_opened {door: composer|next|again|mine, pics}` superseding `look_robes_door`, `source:'robes-build'` on a saved build (the look page never reads its photograph as a frame; her upload keeps the source), the fifth-photographed-piece toast; the home BUILD YOUR OWN row door stays retired (the home cut). **3.3** — the recommendation: the band retires on `_rbConciergeDone() || pieces ≥ 15`, and the meter stops rendering at fifteen even while the band stands. One deviation: the count is FILED pieces (`_waItems.length`, the number the meter itself shows and Style notes reads at 15), not photographed pieces — the ladder cannot retire on a number it never printed. The text below is kept as the original brief for the record.
...
> **As built (2026-09-21, Annie: "jump straight to slice 7 now running option A"; slices 5 and 6 held)**: Option A. `STEPS = ['keypiece', 'style']` — Step 01 · Wardrobe, Step 02 · Style. **7.1 was built as specified and reverted the same day** (Annie: "bring back the original, Splash, Intro, Name — then continue with option A"): the intro reads Begin and the name stage stands for everyone, prefilled when a name is on file; nothing folds into the intro. The intro's promise reads "One piece. Three ways to wear it." The Style step's title reads "While Robes composes your three looks — where does your style sit?" only when a piece was filed (a double skip on the piece lands on the plain question — nothing is composing); the prefire still fires the moment the piece files, before Style. The filed piece's CTA reads Continue (Style is next); the last step's reads Continue to your dashboard and gives its writes a capped beat before navigating. Back from step 01 returns to the name stage. The Cut ledger row falls back to "—". `onboarding_harness` rewritten: 148/148 at both breakpoints.
...
### 7.1 Fold the name into the intro — ~~built~~ reverted 2026-09-21 (Splash → Intro → Name for everyone)
