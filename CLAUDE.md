...
- **Client render (`__tvRenderResult(data, opts)`, `#tv-result-page`, z-index 40; classed CSS `_TV_CSS` → `#tv-style`)** — calendar-first + **styled to the live platform** (amendments 2026-07 refinement): masthead mirrors the Moodboard header (rose eyebrow "The travel edit", short serif-italic `headline` H1) with the packed-progress cluster on the right, then a meta row with a `.weather-strip`-style pill (✈ destination · dates | temp | condition) + the italic location-vibe tag pill. Cream/white cards, hairline borders, `var(--rad*)` radii, 100px pill buttons, `:root` tokens throughout — no dark panels or sharp corners.
