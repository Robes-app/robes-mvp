...
- **Weekly Plan track is live** — `POST /api/weekly` (`WEEKLY_SCHEMA`, gemini-2.5-flash, thinking 0, 7500 tokens, **no imagery** — owned pieces render their wardrobe photos; wardrobe_match resolution identical to `/api/daily`) + client `__wkSubmit`/`__wkRenderResult` (`#wk-result-page`, chronological 5–7 day strip `#wk-strip` + per-day rack `#wk-day`, `.rb-sfoot` footer, crumb "Plan the week"). Saved to lookbook as `type: 'weekly-plan'` with `wkData` (label "Weekly plan" in all lookbook surfaces, reopen via `__snOpenItem` with `{skipSave, savedId}`). The Weekly Planner card lost its "Coming soon" tag — CTA `_cbSetIntent('weekly')` injects "A weekly plan for [my upcoming work week]".
